#include <gui/geom/Order1.hpp>

#include <gui/geom/Crossings.hpp>
#include <gui/geom/Curve.hpp>
#include <gui/geom/PathIterator.hpp>
#include <gui/geom/Rectangle2D.hpp>

#include <math/Math.hpp>

#ifdef __linux__
#include <stdexcept>
#endif

using namespace moduru::gui::geom;
using namespace std;

Order1::Order1(double x0, double y0, double x1, double y1, int direction) : Curve(direction)
{
    this->x0 = x0;
    this->y0 = y0;
    this->x1 = x1;
    this->y1 = y1;
    if(x0 < x1) {
        this->xmin = x0;
        this->xmax = x1;
    } else {
        this->xmin = x1;
        this->xmax = x0;
    }
}

int Order1::getOrder()
{
    return 1;
}

double Order1::getXTop()
{
    return x0;
}

double Order1::getYTop()
{
    return y0;
}

double Order1::getXBot()
{
    return x1;
}

double Order1::getYBot()
{
    return y1;
}

double Order1::getXMin()
{
    return xmin;
}

double Order1::getXMax()
{
    return xmax;
}

double Order1::getX0()
{
    return (direction == INCREASING) ? x0 : x1;
}

double Order1::getY0()
{
    return (direction == INCREASING) ? y0 : y1;
}

double Order1::getX1()
{
    return (direction == DECREASING) ? x0 : x1;
}

double Order1::getY1()
{
    return (direction == DECREASING) ? y0 : y1;
}

double Order1::XforY(double y)
{
    if(x0 == x1 || y <= y0) {
        return x0;
    }
    if(y >= y1) {
        return x1;
    }
    return (x0 + (y - y0) * (x1 - x0) / (y1 - y0));
}

double Order1::TforY(double y)
{
    if(y <= y0) {
        return 0;
    }
    if(y >= y1) {
        return 1;
    }
    return (y - y0) / (y1 - y0);
}

double Order1::XforT(double t)
{
    return x0 + t * (x1 - x0);
}

double Order1::YforT(double t)
{
    return y0 + t * (y1 - y0);
}

double Order1::dXforT(double t, int deriv)
{
    switch (deriv) {
    case 0:
        return x0 + t * (x1 - x0);
    case 1:
        return (x1 - x0);
    default:
        return 0;
    }

}

double Order1::dYforT(double t, int deriv)
{
    switch (deriv) {
    case 0:
        return y0 + t * (y1 - y0);
    case 1:
        return (y1 - y0);
    default:
        return 0;
    }

}

double Order1::nextVertical(double t0, double t1)
{
    return t1;
}

bool Order1::accumulateCrossings(Crossings* c)
{
    auto xlo = c->getXLo();
    auto ylo = c->getYLo();
    auto xhi = c->getXHi();
    auto yhi = c->getYHi();
    if(xmin >= xhi) {
        return false;
    }
    double xstart, ystart, xend, yend;
    if(y0 < ylo) {
        if(y1 <= ylo) {
            return false;
        }
        ystart = ylo;
        xstart = XforY(ylo);
    } else {
        if(y0 >= yhi) {
            return false;
        }
        ystart = y0;
        xstart = x0;
    }
    if(y1 > yhi) {
        yend = yhi;
        xend = XforY(yhi);
    } else {
        yend = y1;
        xend = x1;
    }
    if(xstart >= xhi && xend >= xhi) {
        return false;
    }
    if(xstart > xlo || xend > xlo) {
        return true;
    }
    c->record(ystart, yend, direction);
    return false;
}

void Order1::enlarge(Rectangle2D* r)
{
    r->add(x0, y0);
    r->add(x1, y1);
}

Curve* Order1::getSubCurve(double ystart, double yend, int dir)
{
    if(ystart == y0 && yend == y1) {
        return getWithDirection(dir);
    }
    if(x0 == x1) {
        return new Order1(x0, ystart, x1, yend, dir);
    }
    auto num = x0 - x1;
    auto denom = y0 - y1;
    auto xstart = (x0 + (ystart - y0) * num / denom);
    auto xend = (x0 + (yend - y0) * num / denom);
    return new Order1(xstart, ystart, xend, yend, dir);
}

Curve* Order1::getReversedCurve()
{
    return new Order1(x0, y0, x1, y1, -direction);
}

int Order1::compareTo(Curve* other, vector<double>* yrange)
{
    if(!(dynamic_cast< Order1* >(other) != nullptr)) {
        return super::compareTo(other, yrange);
    }
    auto c1 = dynamic_cast< Order1* >(other);
    if((*yrange)[1] <= (*yrange)[0]) {
        throw std::invalid_argument("yrange already screwed up...");
    }
    (*yrange)[1] = moduru::math::Math::min(moduru::math::Math::min((*yrange)[1], y1), c1->y1);
	if ((*yrange)[1] <= (*yrange)[0]) {
		throw std::invalid_argument("Order1 backstepping from ...."); //)->append((*yrange)[0])->append(u" to "_j)->append((*yrange)[1])->toString());
	}
    if(xmax <= c1->xmin) {
        return (xmin == c1->xmax) ? 0 : -1;
    }
    if(xmin >= c1->xmax) {
        return 1;
    }
    auto dxa = x1 - x0;
    auto dya = y1 - y0;
    auto dxb = c1->x1 - c1->x0;
    auto dyb = c1->y1 - c1->y0;
    auto denom = dxb * dya - dxa * dyb;
    double y;
    if(denom != 0) {
        auto num = ((x0 - c1->x0) * dya * dyb - y0 * dxa * dyb + c1->y0 * dxb * dya);
        y = num / denom;
        if(y <= (*yrange)[0]) {
            y = moduru::math::Math::min(y1, c1->y1);
        } else {
            if(y < (*yrange)[1]) {
                (*yrange)[1] = y;
            }
            y = moduru::math::Math::max(y0, c1->y0);
        }
    } else {
        y = moduru::math::Math::max(y0, c1->y0);
    }
    return orderof(XforY(y), c1->XforY(y));
}

int Order1::getSegment(vector<double>* coords)
{
    if(direction == INCREASING) {
        (*coords)[0] = x1;
        (*coords)[1] = y1;
    } else {
        (*coords)[0] = x0;
        (*coords)[1] = y0;
    }
    return PathIterator::SEG_LINETO;
}
