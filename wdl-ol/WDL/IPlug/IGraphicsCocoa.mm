...
{
    if (shiftPressed && !([pEvent modifierFlags] & NSShiftKeyMask)) {
        shiftPressed = false;
        mGraphics->OnKeyUp(mPrevX, mPrevY, kVK_Shift);
    }
    
    if (ctrlPressed && !([pEvent modifierFlags] & NSControlKeyMask)) {
        ctrlPressed = false;
        mGraphics->OnKeyUp(mPrevX, mPrevY, kVK_Control);
    }
    
    if (altPressed && !([pEvent modifierFlags] & NSAlternateKeyMask)) {
        altPressed = false;
        mGraphics->OnKeyUp(mPrevX, mPrevY, kVK_Option);
    }
    
    if (!shiftPressed && [pEvent modifierFlags] & NSShiftKeyMask) {
        shiftPressed = true;
        mGraphics->OnKeyDown(mPrevX, mPrevY, kVK_Shift);
    }
    
    if (!ctrlPressed && [pEvent modifierFlags] & NSControlKeyMask) {
        ctrlPressed = true;
        mGraphics->OnKeyDown(mPrevX, mPrevY, kVK_Control);
    }
    
    if (!altPressed && [pEvent modifierFlags] & NSAlternateKeyMask) {
        altPressed = true;
        mGraphics->OnKeyDown(mPrevX, mPrevY, kVK_Option);
    }
}
...
- (void)keyUp:(NSEvent *)pEvent
{
    ...
    mGraphics->OnKeyUp(mPrevX, mPrevY, k);
}
...
- (void)keyDown: (NSEvent *)pEvent
{
    ...
    if ([s length] == 1)
    {
        ...
        if (handle)
        {
            ...
            handle = mGraphics->OnKeyDown(mPrevX, mPrevY, k);
        }
