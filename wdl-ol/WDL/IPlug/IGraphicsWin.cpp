...
LRESULT CALLBACK IGraphicsWin::globalKeyboardHookProcedure(int nCode, WPARAM wParam, LPARAM lParam) {
	int processed = FALSE;
	if (nCode == HC_ACTION) {
...
IGraphicsWin::IGraphicsWin(IPlugBase* pPlug, int w, int h, int refreshFPS)
	...
	mHInstance(0)
{
	globalKeyboardHook = SetWindowsHookEx(WH_KEYBOARD_LL, globalKeyboardHookProcedure, mHInstance, NULL);
}
