...
// Default passthrough.
void IPlugBase::ProcessDoubleReplacing(double** inputs, double** outputs, int nFrames, int validOutputs)
{
...
void IPlugBase::MakePreset(char* name, ...)
{
	...
	if (pPreset)
	{
		...
		// We need to store number of public and private params in preset.
		pPreset->mChunk.Put(&numPublicParams);
		pPreset->mChunk.Put(&numPrivateParams);
