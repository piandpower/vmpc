...
public:
    ...
    )
  {
    ...
    if (x1 == x2)
    {
      ...
      for (i = 0; i < fmin(pxon, y2-y); ++i, px += span) DOPIX((LICE_pixel_chan*) px, r, g, b, a, aw)
    }
    else if (y1 == y2)
    {
      int i, x;
      ...
      for (i = 0; i < fmin(pxon, x2-x); ++i, ++px) DOPIX((LICE_pixel_chan*) px, r, g, b, a, aw)
    }
  }
...
static void DoBezierFillSegment(LICE_IBitmap* dest, int x1, int y1, int x2, int y2, int yfill, LICE_pixel color, float alpha, int mode)
{
  ...
  if (x2 == x1)
  {
    int ylo = fmin(y1,yfill);
    int yhi = fmax(y2,yfill);
    if (yhi != yfill) --yhi;
...
static void DoBezierFillSegmentX(LICE_IBitmap* dest, int x1, int y1, int x2, int y2, int xfill, LICE_pixel color, float alpha, int mode)
{
  ...
  if (y2 == y1)
  {
    int xlo = fmin(x1,xfill);
    int xhi = fmax(x2,xfill);
    if (xhi != xfill) --xhi;
