...
int WinIntersectRect(RECT *out, const RECT *in1, const RECT *in2)
{
  ...
  out->left = fmax(in1->left,in2->left);
  out->right = fmin(in1->right,in2->right);
  out->top=fmax(in1->top,in2->top);
  out->bottom = fmin(in1->bottom,in2->bottom);
  
  return out->right>out->left && out->bottom>out->top;
}
void WinUnionRect(RECT *out, const RECT *in1, const RECT *in2)
{
  out->left = fmin(in1->left,in2->left);
  out->top = fmin(in1->top,in2->top);
  out->right=fmax(in1->right,in2->right);
  out->bottom=fmax(in1->bottom,in2->bottom);
}
