	...
	for (int x = 0; x < 248; x++) {
		for (int y = 0; y < 60; y++) {
			if (pixels->at(x).at(y) == true) {
				IRECT r((x*2 * gui_scale) + GetRECT()->L, (y*2 * gui_scale) + GetRECT()->T, (x*2 * gui_scale) + GetRECT()->L + (1* gui_scale), (y*2 * gui_scale) + GetRECT()->T + (1 * gui_scale));
				g->DrawRect(Constants::LCD_HALF_ON(), &r);
				g->DrawPoint(Constants::LCD_ON(), (x * 2 * gui_scale) + GetRECT()->L, (y * 2 * gui_scale) + GetRECT()->T);
			}
		}
	}
	return true;
