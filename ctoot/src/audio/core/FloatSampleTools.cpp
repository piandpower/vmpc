	...
	*/
	...
	for (int inIndex = inOffset; inIndex < endSample; inIndex++, outIndex += outByteStep) {
		...
		case CT_24SB:
			...
			(*output)[outIndex + 1] = static_cast<int32_t>(static_cast<uint32_t>(iSample) >> 8) & 255;
		...
		case CT_32SB:
			...
			(*output)[outIndex + 1] = static_cast<int32_t>(static_cast<uint32_t>(iSample) >> 16) & 255;
			(*output)[outIndex + 2] = static_cast<int32_t>(static_cast<uint32_t>(iSample) >> 8) & 255;
			(*output)[outIndex + 3] = iSample & 255;
			break;
		case CT_32SL:
			iSample = quantize32(input[inIndex] * twoPower31, ditherBits);
			(*output)[outIndex + 3] = iSample >> 24;
			(*output)[outIndex + 2] = static_cast<int32_t>(static_cast<uint32_t>(iSample) >> 16) & 255;
			(*output)[outIndex + 1] = static_cast<int32_t>(static_cast<uint32_t>(iSample) >> 8) & 255;
			(*output)[outIndex] = iSample & 255;
			break;
		default:
