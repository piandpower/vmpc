...
ShortMessage::ShortMessage(vector<char> data) : MidiMessage(data) {
}
...
void ShortMessage::setMessage(int command, int channel, int data1, int data2) {
	if (command >= 0xF0 || command < 0x80) {
		//string error = "command out of range: 0x" + Integer.toHexString(command));
...
void ShortMessage::setMessage(vector<char> data, int length) {
	this->data = data;
	this->length = length;
}
