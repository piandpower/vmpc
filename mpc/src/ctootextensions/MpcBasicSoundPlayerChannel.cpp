...
MpcBasicSoundPlayerChannel::MpcBasicSoundPlayerChannel(weak_ptr<MpcBasicSoundPlayerControls> controls)
{
	...
	auto sc = lMixer->getMixerControls()->getStripControls("65");
	auto cc = dynamic_pointer_cast<ctoot::audio::mixer::MainMixControls>(sc->getControls()[4]);
	fader = dynamic_pointer_cast<ctoot::audio::fader::FaderControl>(cc->getControls()[2]).get();
}
...
{
	auto lMixer = mixer.lock();
	lMixer->getStrip("65").lock()->setInputProcess(voice.lock().get());
}
